...
export const wordPairRouter = express.Router();

wordPairRouter.get("/", (req, res) => {
	res.status(400).json({ error: "Missing parameters" });
});

wordPairRouter.get("/:date", async (req, res) => {
	try {
		let dateObj = null;
		try {
		...
		const result = JSON.parse(await wo.get_word_pair_occ(allWordsTT, "json", 25));
